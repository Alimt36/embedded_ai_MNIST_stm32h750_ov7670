...
// //   // ---> diagnostic : skip the scan entirely, hardcode address, see if UART survives
// //   HAL_UART_Transmit(&huart3, (uint8_t*)"BEFORE_OLED\r\n", 13, 100);
...
// //   HAL_UART_Transmit(&huart3, (uint8_t*)"AFTER_OLED\r\n", 12, 100);
...
// //   // ---> CubeMX did not generate AF config for I2C4 pins despite correct assignment
// //   //      in the Pinout view, added manually here so it survives regeneration
// //   GPIO_InitTypeDef GPIO_InitStruct_I2C4 = {0};
// //   GPIO_InitStruct_I2C4.Pin       = GPIO_PIN_12 | GPIO_PIN_13;   // ---> PD12 = SCL, PD13 = SDA
// //   GPIO_InitStruct_I2C4.Mode      = GPIO_MODE_AF_OD;             // ---> open-drain, required for I2C
// //   GPIO_InitStruct_I2C4.Pull      = GPIO_PULLUP;                 // ---> internal pull-up, still want external 4.7k if unreliable
...
//                              (this is the supervisor-facing hardware demo mode)
//---------------------------------------------------------------------------------------------------------------------------
#define CAMERA_UART_DEBUG
// #define CAMERA_OLED_DISPLAY

#if defined(CAMERA_UART_DEBUG) && defined(CAMERA_OLED_DISPLAY)
    #error "pick only ONE mode, not both"
#endif
#if !defined(CAMERA_UART_DEBUG) && !defined(CAMERA_OLED_DISPLAY)
    #error "pick exactly one mode"
#endif

#ifdef CAMERA_OLED_DISPLAY
#include "ssd1306.h"
#endif
/* USER CODE END Includes */
...
#ifdef CAMERA_UART_DEBUG
    // ---> full debug stream : result + 28x28 buffer + raw frame, every loop
    uint8_t result_buf[20];
    sprintf((char*)result_buf, "DIGIT:%d\r\n", predicted);
    HAL_UART_Transmit(&huart3, result_buf, strlen((char*)result_buf), 100);
...
#ifdef CAMERA_OLED_DISPLAY
    // ---> hardware display : 28x28 preview + predicted digit on the OLED, plus a
    //      lightweight single-line UART result (no raw frame spam in this mode)
    if(oled_addr != 0)
    {
        SSD1306_Clear();
        SSD1306_DrawGrayBuffer(small_buf, 0, 0, 2);
